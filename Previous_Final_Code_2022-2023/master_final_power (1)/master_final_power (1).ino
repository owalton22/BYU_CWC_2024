...
#define LOAD_NET_SWITCH_2_PIN 42
#define LOAD_NET_SWITCH_4_PIN 40
#define LOAD_NET_SWITCH_8_PIN 44
#define ACTUATOR_BRAKE_SWITCH_SIGNAL_PIN 13
#define BRAKE_PIN 8
#define EBRAKE_BTN_PIN 14
...
#define BRAKE_ENGAGED 10
#define BRAKE_DISENGAGED 30
...
float sampledRPM[sampleSize] = {0, 0};
float sampledVoltage[sampleSize] = {0, 0};
float sampledPower[sampleSize] = {0, 0};
...
bool entering_survival = true;

bool load_is_disconnected = false;

bool is_first_time_in_estop = true;
...
float resistorRPMPLUT[3][7] = {  
   {  5.0,   6.0,    7.0,   8.0,      9.0,   10.0,   11.0 }, // m/s 
   {  12,     8,      6,     12,      12,      0,     9   }, // resistor index
   {1400.0, 1630.0, 1940.0, 2300.0, 2550.0, 2880.0, 3180.0} // rpm
};

float resistorPWRLUT[3][7] = {  
   {  5.0,   6.0,    7.0,    8.0,     9.0,   10.0,  11.0 }, // m/s 
   {   12,     8,      6,     12,      12,      0,     0 }, // resistor index
   {  2.0,   3.5,    5.5,    7.5,     9.5,    16.5,  19.0 } // power value to trigger transition to next wind speed
};
...
//            set_pitch(INITIAL_PITCH);
    ...
    case restart:
        ...
        // } else if ((is_E_stop() || !is_load_connected() && (avgPower >= 0.01)) && !is_first_time_in_restart) {
        } else if (is_E_stop() || !is_load_connected() && (avgPower >= 0.01)) {
            Serial.println("Entering ESTOP State!");
            entering_e_stop = true;
            // is_first_time_in_restart = false;
            currentState = emergency_shutdown;
        // } else if (avgPower >= POWER_CURVE_RPM && avgVoltage >= POWER_CURVE_VOLTAGE) {
        } else if (avgPower >= PWR_CRV_PWR) {
            Serial.println("Entering POWER_CURVE State!");
            // is_first_time_in_restart = false;
            currResistorIndex = resistorPWRLUT[1][0];
    ...
    case power_curve:
        ...
        if (avgPower >= TARGET_MAX_PWR && windSpeed == 11){
            ...
            //TARGET_MAX_RPM = rpm_at_11 - rpm_at_11 * STEADY_POWER_RANGE_PERCENT;
            digitalWrite(ACTUATOR_BRAKE_SWITCH_SIGNAL_PIN, HIGH);
    ...
    case steady_power:
        if (test_state == toggle_test) {
            ...
            entering_e_stop = true;
            currentState = emergency_shutdown;
        } else if ((current_angle < SURVIVAL_PITCH_ANGLE_THRESHOLD) || (avgPower > (TARGET_MAX_PWR * 1.3))){
            Serial.println("Entering SURVIVAL State!");
            digitalWrite(ACTUATOR_BRAKE_SWITCH_SIGNAL_PIN, LOW);
            entering_survival = true;
    ...
    case emergency_shutdown:
        ...
        // if (!is_E_stop() && is_load_connected() || (avgPower > 0.01)) {
        if (!is_E_stop() && (is_load_connected() || !load_is_disconnected)) {
            Serial.println("Entering RESTART State!");
            external_power_switch();
            brakeControl.write(BRAKE_DISENGAGED);
            is_brake_engaged = false;
            load_is_disconnected = false;
            windSpeed = 4;
            set_pitch(INITIAL_PITCH);
            set_load(INITIAL_LOAD);
            delay(1000);
    ...
    case power_curve:
        if (avgPower > TARGET_MAX_PWR) {
            windSpeed++;
            char strBuf[50];
            sprintf(strBuf, "Wind speed increased to %d m/s", windSpeed);
            Serial.println(strBuf);
      
            // TARGET_MAX_RPM = resistorRPMPLUT[2][windSpeed - 5];
            // currResistorIndex = resistorRPMLUT[1][windSpeed - 5];

            TARGET_MAX_PWR = resistorPWRLUT[2][windSpeed - 5];
            currResistorIndex = resistorPWRLUT[1][windSpeed - 5];
            set_load(currResistorIndex);
...
void optimize_pitch() {
  ...
  if (avgPower < TARGET_MIN_PWR) {
    ...
    set_pitch_steady_power(current_angle+1);
  }
...
void set_pitch_steady_power(int pitch_angle) {
...
// If we aren't getting the pitch and brake set before capacitor runs out then
// maybe we could try setting brake first than adjusting pitch, or at a threshold pitch
void engage_ebrake() {
